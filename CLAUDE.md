...
- `skills/use-ravion/SKILL.md` is a mirror. Its source of truth is `packages/web/public/SKILL.md` in the website repository, served at `https://www.ravion.com/SKILL.md` and listed in `https://www.ravion.com/.well-known/agent-skills/index.json`. Edit it there. `.github/workflows/sync-hosted-skill.yml` runs every three hours (and on `workflow_dispatch` or a `hosted-skill-updated` repository dispatch), pulls `SKILL.md` plus every reference file it links to, bumps the patch version of all four plugin manifests, and commits straight to `main`, so this mirror never drifts from the hosted copy. Local edits to `skills/use-ravion/` are overwritten by the next sync.
